...
int GMRES(SLAE &slae, int m, int maxIter, double eps)
{
   ...
   initSizeOfMatrix(V, sizeMatrix, m); 
   initSizeOfMatrix(H, m + 1, m);      
   ...
   double normDiscrepancy = calcEuclideanNorm(r);
   double normRightPart = calcEuclideanNorm(bTemp);
   ...
   for (int k = 1; k < maxIter && relativeDiscrepancy > eps; k++)
   {
      ...
      for (int mu = 0; mu < mTemp && !elemHisZero; mu++)
      {
         ...
         H[mu + 1][mu] = calcEuclideanNorm(vAdditional);
      ...
      calcVecPlusCoefVec(xTemp, xTemp, 1., Vz);

      calcX(LU, xTemp, q);
      calcVectorDiscrepancy(slae, r);
      calcY(LU, r, r);

      normDiscrepancy = calcEuclideanNorm(r);
...
void initFirstColumn(vector<vector<double>> &V, vector<double> &r)
{
   ...
   double normR = calcEuclideanNorm(r);
...
void calcAdditionalVector(vector<vector<double>> &H, vector<vector<double>> &V, vector<double> &vAdditional, int mu)
{
   ...
   for (int lambda = 0; lambda <= mu; lambda++)
   {
      ...
      calcVecPlusCoefVec(vAdditional, vAdditional, -1., temp);
   }
}